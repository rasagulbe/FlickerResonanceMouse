function [y,w,regressline,r]=nt_detrend_edited(x,order,w,basis,thresh,niter,wsize,step)
...
if isempty(wsize) || ~wsize;
    ...
    if ~(isempty(w) || (isnumeric(w) && size(w,1)==size(x,1))) ; disp(size(w)); error('!'); end
...
%             start_all = [start_all start]; % #RG commented out
%             stop_all = [stop_all stop];    % #RG commented out
            wsize2=stop-start+1;
...
%             offset=offset+step; %RG added
%             if offset>size(x,1)-step; break; end
%             offset_all = [offset_all offset ]; %RG test
        
        end
        % #RG added: Plot to test
%         figure
%         plot(start_all,'bo-')
%         hold on
%         plot(stop_all,'ro-')
%         plot(offset_all,'mo-')
        ...
        % find outliers
        d=x-trend; % RG: trend is regression line 
   ...
   regressline = reshape(trend,dims);
